...
This article shares eight practical tips for writing an effective `CLAUDE.md`. Key principles include keeping it concise, defining forbidden technologies, using actionable rules, linking to documentation instead of embedding it, adding module-specific guidance, enforcing standards with hooks, maintaining project memory, and encoding workflow preferences to improve Claude Code's project understanding.
...
```markdown
# ❌ 模糊——Claude 无法执行## Coding Rules- 写干净的代码
- 保持简洁
- 注重性能
  
# ✅ 具体——Claude 可以直接执行## Coding Rules- 使用 named export（路由文件除外）
- 禁止 any 类型，用泛型或接口替代
...
```markdown
# src/auth/CLAUDE.md
## 安全红线- 绝不修改 token 验证逻辑，除非明确要求且经过 review
- 绝不引入新的认证方式而不更新测试
- 所有认证相关变更必须通过 `pnpm test src/auth` 全部测试

## 已知陷阱- Magic link 生成依赖 `crypto.randomUUID()`，不要换成其他随机方法
- Session 存储在 Redis，不是内存——重启不会丢失
```

## **六、让 CLAUDE.md 驱动 Hook，而不是靠记忆**
...
```markdown
## Hooks & Quality Gates以下规则由 `.claude/hooks/` 强制执行，不是提醒：
...
![HHpQp0Ra0AACeak](https://img-reg-ab.imagency.cn/e/464601d3b64038fff6a93f9d78b30b21.png)
## **现在可以做的事**
